...
func (conn *DB) AddUser(user utils.User) error {
	...
	defer stmt.Close()
	res, err := stmt.Exec(user.UserID, user.Password, user.FirstName, user.LastName)
	if err != nil {
		return err
	}
...
func (conn *DB) Get(userID string) string {
	...
	var name4 string
	for query.Next() {
		if err := query.Scan(&name, &name2, &name3, &name4); err != nil {
			fmt.Println("err", err)
		}
		fmt.Println(name, name2, name3, name4, "end")
	}
	return fmt.Sprintf("ID: %s.\nName: %s %s.", name, name3, name4)
}
